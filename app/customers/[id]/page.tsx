...
import { ArrowLeft, Loader2, Pencil, FileText, TrendingUp, Phone, Mail, MapPin, Hash, Save, X } from 'lucide-react';
      ...
      <div className="space-y-6 max-w-4xl">
        ...
        <div className="flex items-center justify-between flex-wrap gap-3">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              {customer.taxId && <p className="text-slate-500 text-sm">VKN: {customer.taxId}</p>}
            </div>
          </div>
          <div className="flex gap-2">
            ...
            <Link href={`/quotes/new?customerId=${customer.id}`} className="flex items-center gap-2 px-3 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-lg text-sm font-medium transition-colors">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="bg-white rounded-xl p-4 shadow-sm border-l-4 border-blue-500">
            ...
            <p className="text-xl font-bold text-slate-800">{formatCurrency(customer.totalInvoiced)}</p>
          </motion.div>
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.05 }} className="bg-white rounded-xl p-4 shadow-sm border-l-4 border-green-500">
            <p className="text-xs text-slate-500 mb-1">Toplam Tahsilat</p>
            <p className="text-xl font-bold text-green-600">{formatCurrency(customer.totalPaid)}</p>
          </motion.div>
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className={`bg-white rounded-xl p-4 shadow-sm border-l-4 ${customer.balance > 0 ? 'border-orange-500' : 'border-slate-300'}`}>
            <p className="text-xs text-slate-500 mb-1">Cari Bakiye (Alacak)</p>
            <p className={`text-xl font-bold ${customer.balance > 0 ? 'text-orange-600' : 'text-slate-600'}`}>{formatCurrency(customer.balance)}</p>
          </motion.div>
        </div>
        ...
        <div className="bg-white rounded-xl shadow-sm p-5">
          <div className="flex items-center justify-between mb-4">
            <h2 className="font-semibold text-slate-700">Müşteri Bilgileri</h2>
            {editing && (
              <div className="flex gap-2">
          ...
          {editing ? (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm">
              {customer.phone && <div className="flex items-center gap-2 text-slate-600"><Phone className="w-4 h-4 text-slate-400" />{customer.phone}</div>}
              {customer.email && <div className="flex items-center gap-2 text-slate-600"><Mail className="w-4 h-4 text-slate-400" />{customer.email}</div>}
              {customer.taxId && <div className="flex items-center gap-2 text-slate-600"><Hash className="w-4 h-4 text-slate-400" />VKN: {customer.taxId}</div>}
              {customer.address && <div className="flex items-center gap-2 text-slate-600 sm:col-span-2"><MapPin className="w-4 h-4 text-slate-400 flex-shrink-0" />{customer.address}</div>}
              {customer.notes && <div className="sm:col-span-2 text-slate-500 italic text-xs">{customer.notes}</div>}
            </div>
          )}
        </div>
        ...
        {customer.invoices?.length > 0 && (
          <div className="bg-white rounded-xl shadow-sm overflow-hidden">
            <div className="p-4 border-b">
              <h2 className="font-semibold text-slate-700">Faturalar</h2>
            </div>
            <div className="divide-y">
              {customer.invoices.map((inv: any) => (
                <div key={inv.id} className="p-4 flex items-center justify-between">
                  <div>
                    <p className="font-medium text-slate-700 text-sm">{inv.invoiceNo}</p>
                    <p className="text-xs text-slate-400">{new Date(inv.date).toLocaleDateString('tr-TR')}</p>
                  </div>
                  <div className="flex items-center gap-3">
                    <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${statusColor[inv.status]}`}>{statusLabel[inv.status]}</span>
                    <span className="font-semibold text-slate-700 text-sm">{formatCurrency(inv.total)}</span>
                  </div>
                </div>
              ))}
            </div>
