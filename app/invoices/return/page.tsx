    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-md">
        <div className="bg-red-600 rounded-t-2xl px-5 py-4 flex items-center justify-between">
          <h3 className="text-white font-semibold text-base">{item.description || 'İade Ürün / Hizmet'}</h3>
        ...
        <div className="p-5 space-y-4">
          ...
          <div className="bg-red-50 border border-red-200 rounded-xl p-4">
            ...
            <div className="flex justify-between items-center border-t border-red-200 pt-2 mt-1">
              <span className="text-sm font-bold text-slate-700">TOPLAM (İade)</span>
          ...
          <button type="button" onClick={() => { if (item.description || item.unitPrice) onConfirm(item); }}
            ...
            <Plus className="w-4 h-4" /> İade Kalemine Ekle
          </button>
        </div>
      </div>
    </div>
  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
      ...
      <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-sm p-6">
        ...
        <p className="text-xs text-slate-400 mb-5">Devam etmek istiyor musunuz?</p>
        <div className="flex gap-3">
          <button onClick={onCancel} className="flex-1 py-2 border border-slate-200 rounded-lg text-sm text-slate-600 hover:bg-slate-50">İptal</button>
          <button onClick={onConfirm} className="flex-1 py-2 bg-amber-500 hover:bg-amber-600 text-white rounded-lg text-sm font-medium">Yine de Devam Et</button>
        </div>
      </div>
    </div>
  );
      ...
      <div className="space-y-3 h-full">
        ...
        <div className="flex gap-4 items-start flex-col lg:flex-row">
          ...
          <div className="w-full lg:w-[340px] lg:flex-shrink-0 bg-white rounded-xl shadow-sm overflow-hidden">
            <div className="bg-red-600 px-4 py-3">
              <p className="text-white font-semibold text-sm truncate">
                {selectedCustomer?.name ? `${selectedCustomer.name} — İade` : 'İade Faturası'}
              </p>
            </div>
            <div className="p-4 space-y-3">
              ...
              <div className="flex items-start gap-2 bg-red-50 border border-red-200 rounded-lg px-3 py-2 text-xs text-red-700">
                ...
                İade faturası müşteri bakiyesini azaltır ve stok iade ürünleri geri ekler.
              </div>
              <div>
              ...
              <div>
                ...
                <input value={form.invoiceNo} onChange={e => setField('invoiceNo', e.target.value)} placeholder="Otomatik oluşturulur (IAD-...)"
                  className="w-full px-3 py-2 border border-slate-200 rounded-lg text-sm focus:ring-2 focus:ring-red-400 outline-none" />
              </div>
              <div>
          ...
          <div className="flex-1 min-w-0 bg-white rounded-xl shadow-sm overflow-hidden">
            <div className="bg-red-600 px-4 py-3">
              <p className="text-white font-semibold text-sm uppercase tracking-wide">İade Edilen Ürün / Hizmetler</p>
            </div>
            <div className="p-4 space-y-4">
              ...
              <button type="button" onClick={() => openNewModal()}
                ...
                <Plus className="w-4 h-4" /> Manuel İade Kalemi Ekle
              </button>
                ...
                <div className="border-t pt-4 space-y-2">
                  <div className="flex items-center justify-between">
                    ...
                    <select value={form.vatRate} onChange={e => setField('vatRate', e.target.value)}
                      className="px-3 py-1.5 border border-slate-200 rounded-lg text-sm focus:ring-2 focus:ring-red-400 outline-none bg-white w-24">
                      {VAT_RATES.map(r => <option key={r} value={r}>%{r}</option>)}
                    </select>
                  </div>
                  <div className="flex justify-between text-sm text-slate-600">
                    <span>Ara Toplam (İade)</span>
                    <span className="text-red-600">-{subtotal.toLocaleString('tr-TR', { minimumFractionDigits: 2 })} {form.currency}</span>
                  </div>
                  {vatRate > 0 && (
                    <div className="flex justify-between text-sm text-slate-600">
                      <span>KDV (%{vatRate})</span>
                      <span className="text-red-600">-{vatAmount.toLocaleString('tr-TR', { minimumFractionDigits: 2 })} {form.currency}</span>
                    </div>
                  )}
              ...
              {items.length === 0 && (
                <div className="text-center py-10 border-2 border-dashed border-red-100 rounded-xl text-slate-400 text-sm">
                  İade edilecek ürünleri arayın veya "Manuel İade Kalemi Ekle" butonunu kullanın
                </div>
              )}
            </div>
          </div>
