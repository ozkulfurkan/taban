...
import { Calculator, Plus, Trash2, Loader2, Save, ChevronDown, ChevronUp, Paintbrush, Wrench, ArrowLeft } from 'lucide-react';
...
export default function EditCalculationPage() {
  ...
  const SectionHeader = ({ id: sId, title, icon: Icon }: { id: string; title: string; icon: any }) => (
    <button
      onClick={() => setExpandedSection(expandedSection === sId ? '' : sId)}
    ...
    >
      ...
      {expandedSection === sId ? <ChevronUp className="w-5 h-5 text-slate-400" /> : <ChevronDown className="w-5 h-5 text-slate-400" />}
    </button>
  );
    ...
    <AppShell>
      <div className="space-y-6 max-w-4xl">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-bold text-slate-800">{t('calculation', 'editCalculation')}</h1>
